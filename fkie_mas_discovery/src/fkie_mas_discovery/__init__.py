...
def main():
    ...
    try:
        discoverer = mas_discovery.Discoverer(mcast_port, mcast_group, rpc_port, rpc_addr=rpc_addr, ws_port=ws_port)
        discoverer.start()
        rospy.spin()
        discoverer.finish()
    except Exception as e:
        ...
        Log.error(f"Error while starting mas_discovery: {e}", traceback.format_exc())
