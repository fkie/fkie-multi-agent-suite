...
class DiscoverSocket(socket.socket):
    ...
    def __init__(self, port, mgroup, ttl=20, send_mcast=True, listen_mcast=True):
        ...
        if self.unicast_only:
            ...
            Log.warn("Multicast disabled! This master is only by unicast reachable!")
        if self.interface:
        ...
        if not self.unicast_only:
            Log.debug("Create multicast socket at ('%s', %d)", self.mgroup, port)
            ...
            if hasattr(socket, "SO_REUSEPORT"):
                try:
                    ...
                    Log.warn("SO_REUSEPORT failed: Protocol not available, some functions are not available.")
            ...
            try:
                ...
                msg = str(errobj)
                Log.fatal("Unable to bind multicast to interface: %s, check that it exists: %s", self.mgroup, msg)
                raise
            ...
            if self.listen_mcast:
                ...
                if self.addrinfo[0] == socket.AF_INET:  # IPv4
                    ...
                    if not self.interface_ip:
                        self.group_bin = socket.inet_pton(socket.AF_INET, self.mgroup) + struct.pack(
                            "=I", socket.INADDR_ANY
                        )
                        self.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, self.group_bin)
                    else:
                        self.group_bin = socket.inet_aton(self.mgroup) + socket.inet_aton(self.interface_ip)
                        self.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_IF, socket.inet_aton(self.interface_ip))
                        self.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, self.group_bin)
                else:  # IPv6
                    # Create group_bin for de-register later
                    # Set socket options for multicast
                    self.group_bin = socket.inet_pton(self.addrinfo[0], self.mgroup) + struct.pack(
                        "@I", socket.INADDR_ANY
                    )
                    self.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_JOIN_GROUP, self.group_bin)
        except OSError as errobj:
            msg = str(errobj)
        ...
        if not self.unicast_only:
            if self.listen_mcast:
                if self.addrinfo[0] == socket.AF_INET:  # IPv4
                    ...
                    self.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_LEAVE_GROUP, self.group_bin)
            Log.debug("Close multicast socket at ('%s', %s)", self.mgroup, self.port)
                ...
                if send_item.destinations:
                        ...
                        try:
                            if addr in self._locals:
                                self.receive_queue.put(
                                    QueueReceiveItem(msg, (addr, self.port), QueueReceiveItem.LOOPBACK), timeout=1
                                )
                    ...
                    try:
                        if not self.listen_mcast:
                            self.receive_queue.put(
                                QueueReceiveItem(msg, ("localhost", self.port), QueueReceiveItem.LOOPBACK), timeout=1
                            )
    ...
    def localifs():
        ...
        if _use_netifaces:
            ...
            outbytes = struct.unpack(
                "iL", fcntl.ioctl(sockfd.fileno(), SIOCGIFCONF, struct.pack("iL", MAXBYTES, names.buffer_info()[0]))
            )[0]
            namestr = names.tostring()
            return [
                (namestr[i : i + var1].split("\0", 1)[0], socket.inet_ntoa(namestr[i + 20 : i + 24]))
    ...
    def recv_loop_multicast(self):
        ...
        while not rospy.is_shutdown() and not self._closed:
            try:
                ...
                Log.warn("Error while process recevied multicast message: %s", full_error)
            except OSError:
                import traceback
        ...
        if self.unicast_socket is not None:
            while not rospy.is_shutdown() and not self._closed:
                try:
                    ...
                    Log.warn("Error while process recevied unicast message: %s", full_error)
                except OSError:
                    import traceback

                    Log.warn("unicast socket error: %s", traceback.format_exc())
...
class UcastSocket(socket.socket):
    def __init__(self, interface, port):
        ...
        if not self.interface:
            ...
            for iface in ifaces:
                if not (iface.startswith("127") or iface.startswith("::1")):
                    ...
                    break
            Log.info("+ Bind to unicast socket @(%s:%s)", self.interface, port)
            addrinfo = UcastSocket.getaddrinfo(self.interface)
        else:
            # Otherwise get the address info for the interface specified.
            Log.info("+ Bind to specified unicast socket @(%s:%s)", self.interface, port)
            addrinfo = UcastSocket.getaddrinfo(self.interface)
        ...
        try:
            ...
            msg = str(errobj)
            Log.fatal("Unable to bind unicast to interface: %s, check that it exists: %s", self.interface, msg)
            raise
    ...
    def send2addr(self, msg, addr):
        ...
        try:
            ...
            if errobj.errno in [-5]:
                if addr not in self.sock_5_error_printed:
                    Log.warn("socket.error[%d]: %s, addr: %s", errobj.errno, msg, addr)
                    self.sock_5_error_printed.append(addr)
