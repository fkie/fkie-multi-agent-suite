...
class SensorInterface:
    ...
    def add_sample(self, values: dict[str, float], ts: float | None = None) -> float:
        """
    ...
    def _start_check_sensor(self):
        ...
        try:
            ...
            nmd.ros_node.get_logger().debug("%s: error while check_sensor(): %s" % (type(self).__name__, err))
        if self.is_active() and self._interval > 0:
            ...
            self.start_timer(max(0.0, self._interval - elapsed), self._start_check_sensor)
    ...
    def update_value_last_ts(self, msg, nowts: float = 0, ts: float = 0):
        ...
        if msg.values and msg.values[-1].key == "Timestamp":
            del msg.values[-1]
        msg.values.append(KeyValue(key="Timestamp", value=formats.timestamp_fmt(ts, False, False)))
