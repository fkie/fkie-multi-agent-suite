...
class NetLoad(SensorInterface):
    ...
    def __init__(self, hostname='', interval=3.0, warn_level=0.9, window=10.0):
        ...
        SensorInterface.__init__(
            self, hostname, sensorname='Network Load', interval=interval, window=window)
    ...
    def check_sensor(self):
        ...
        diag_msg = 'warn at >%.2f%% at %.0fMBit (avg over %.0fs)' % (
            self._net_load_warn * 100.0, self._net_speed, self.window)
        ...
        for net_if, net_if_stats in net_stats.items():
            ...
            bytes_sent_1s = max(
                0.0, (net_values.bytes_sent - stat_last[0]) / duration)
            bytes_recv_1s = max(
                0.0, (net_values.bytes_recv - stat_last[1]) / duration)
        ...
        for net_if in parsed_interfaces:
            ...
            diag_vals.append(
                KeyValue(key='%s: sent [1s]' % net_if, value='%.2f' % sent_avg))
            diag_vals.append(
                KeyValue(key='%s: recv [1s]' % net_if, value='%.2f' % recv_avg))
            ...
            if percent_sent >= warn_level or percent_recv >= warn_level:
                ...
                if percent_sent >= warn_level and percent_recv >= warn_level:
                    diag_msg = 'Net load for sent is %.0f%% and recv %.0f%% (warn >%.0f%% [%dMBit], avg over %.0fs)' % (
                        percent_sent * 100, percent_recv * 100, self._net_load_warn * 100, self._net_speed, window_span)
                elif percent_sent >= warn_level:
                    diag_msg = 'Net load for sent is %.0f%% (warn >%.0f%% [%dMBit], avg over %.0fs)' % (
                        percent_sent * 100, self._net_load_warn * 100, self._net_speed, window_span)
                else:
                    diag_msg = 'Net load for recv is %.0f%% (warn >%.0f%% [%dMBit], avg over %.0fs)' % (
                        percent_recv * 100, self._net_load_warn * 100, self._net_speed, window_span)
