        ...
        if _cache_ts > 0 and (now - _cache_ts) < max_age:
            ...
            try:
                for proc in psutil.process_iter(attrs=['name', 'cpu_percent']):
                    try:
                        result.append((proc.info['cpu_percent'] or 0.0,
                                       proc.info['name'] or '', proc.pid))
                ...
                nmd.ros_node.get_logger().debug(
                    "can not determine process load: %s" % err)
...
def format_process_load(min_percent: float = 0.0, count: int = 3,
                        normalized: bool = False, max_age: float = 1.0) -> List[str]:
    '''
