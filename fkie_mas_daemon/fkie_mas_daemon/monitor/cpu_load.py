...
class CpuLoad(SensorInterface):
    def __init__(self, hostname: str = "", interval: float = 5.0, warn_level: float = 0.9, window: float = 10.0):
        ...
        SensorInterface.__init__(self, hostname, sensorname="CPU Load", interval=interval, window=window)
    ...
    def check_sensor(self):
        ...
        diag_vals = []
        diag_msg = "warn at >%.2f%% (avg over %.0fs)" % (self._cpu_load_warn * 100.0, self.window)
        ...
        self.add_sample({"cpu%d" % cpu_idx: cpu_percent for cpu_idx, cpu_percent in enumerate(cpu_percents)}, ts=now)
        ...
        count_warn_cpu = len([value for value in core_avgs if value / 100.0 >= warn_level])
        window_span = self.window_span()
        diag_vals.append(KeyValue(key="Max [%]", value="%.2f" % cpu_max_percent))
        diag_vals.append(KeyValue(key="Avg [%]", value="%.2f" % cpu_avg_percent))
        ...
        if count_warn_cpu > 1:
            ...
            diag_msg = "CPU load of %d cores is >%.0f%% (avg over %.0fs)" % (
                count_warn_cpu,
                self._cpu_load_warn * 100,
                window_span,
            )
            # determine processes with high load, values are shared with the
            # CPU temperature sensor
            for msg in format_process_load(
                min_percent=warn_level * 100.0, count=self._count_processes, normalized=True, max_age=self._interval
            ):
