...
class HddUsage(SensorInterface):
    ...
    def __init__(self, hostname='', interval=30.0, warn_level=0.95, window=10.0):
        ...
        SensorInterface.__init__(
            self, hostname, sensorname='HDD Usage', interval=interval, window=window)
    ...
    def check_sensor(self):
        ...
        try:
            ...
            diag_msg = 'warn at >%s%% (<%s, avg over %.0fs)' % (
                self._hdd_usage_warn * 100., formats.sizeof_fmt(warn_on_space), self.window)
            ...
            self.add_sample({'free': float(hdd.free),
                             'free_percent': 100.0 - hdd.percent})
            ...
            if free_avg <= warn_level:
                ...
                diag_msg = 'Free disk space on log path only %s (warn on <%s, avg over %.0fs)' % (
                    formats.sizeof_fmt(free_avg), formats.sizeof_fmt(warn_on_space), window_span)
