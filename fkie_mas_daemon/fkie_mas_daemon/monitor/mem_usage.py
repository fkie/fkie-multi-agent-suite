...
class MemUsage(SensorInterface):
    def __init__(self, hostname="", interval=5.0, warn_level=0.95, window=10.0):
        ...
        SensorInterface.__init__(self, hostname, sensorname="Memory Usage", interval=interval, window=window)
    ...
    def check_sensor(self):
        ...
        diag_msg = "warn at >%s%% (<%s, avg over %.0fs)" % (
            self._mem_usage_warn * 100.0,
            formats.sizeof_fmt(warn_on_mem),
            self.window,
        )
        ...
        self.add_sample({"free": mem_free, "free_percent": mem_free * 100.0 / float(mem.total)})
