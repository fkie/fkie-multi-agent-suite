...
from fkie_mas_pylib import names, ros_pkg
...
class LaunchConfig:
    ...
    def __init__(self, launch_file, package=None, masteruri="", host="", argv=None, monitor_servicer=None):
        ...
        self.__masteruri = masteruri if masteruri else ros1_masteruri.from_master(True)
        self.__roscfg = None
        self.argv = argv
    ...
    def filename(self):
        ...
        raise LaunchConfigException("launch file %s not found!" % self.__launch_file)
    ...
    def load(self, argv):
        ...
        try:
            ...
            for n in roscfg.nodes:
                ...
                associations_param = names.ns_join(node_fullname, "nm/associations")
                if associations_param in roscfg.params:
                    ...
                    Log.warn(
                        "'nm/associations' is deprecated, use 'mas/associations'! found for node: %s in %s"
    ...
    def _replace_arg(self, arg, argv_defaults, argv_values):
        ...
        while arg_match is not None:
            ...
            if endIndex > -1:
                ...
                if rec_inc > 100:
                    raise LaunchConfigException(
                        "Can't resolve the argument `%s` in `%s` argument: recursion depth of 100 reached!"
                        % (arg_name, arg)
                    )
                ...
                if param.endswith("/robots"):
                    if isinstance(p.value, list):
                        if len(p.value) > 0 and len(p.value[0]) != 5:
                            Log.warn(
                                "WRONG format, expected: ['host', 'type', 'name', 'images', 'description'] -> ignore; param: %s"
                                % param
                            )
                        else:
                            for entry in p.value:
                                self._robot_description[entry[0]] = {
                                    "type": entry[1],
                                    "name": entry[2],
                                    "images": entry[3].split(","),
        ...
        if self.roscfg is not None:
                ...
                if param.endswith("capabilities"):
                    if isinstance(p.value, list):
                        if len(p.value) > 0 and len(p.value[0]) != 4:
                            Log.warn(
                                "WRONG format, expected: ['name', 'type', 'images', 'description'] -> ignore; param: %s"
                                % param
                            )
                        else:
                            for entry in p.value:
            ...
            for item in self.roscfg.nodes:
                ...
                while cap_param not in self.roscfg.params and cap_param.count(names.SEP) > 1:
                    cap_ns = names.namespace(cap_ns, with_sep_suffix=True, global_on_none=True)
                        ...
                        if p.value not in result[machine_name][ns]:
                            try:
                                result[machine_name][ns][p.value] = {
                                    "type": capabilies_descr[p.value]["type"],
