...
class MemUsage(SensorInterface):
    ...
    def check_sensor(self):
        ...
        diag_msg = "warn at >%s%% (<%s)" % (self._mem_usage_warn * 100.0, formats.sizeof_fmt(warn_on_mem))
        ...
        if mem.total - mem.used <= warn_on_mem:
            ...
            diag_msg = "Memory available %s (warn <%s)" % (
                formats.sizeof_fmt(mem.total - mem.used),
        ...
        diag_vals.append(
            KeyValue(key="Free [%]", value="%.2f" % (float(mem.total - mem.used) * 100.0 / float(mem.total)))
        )
