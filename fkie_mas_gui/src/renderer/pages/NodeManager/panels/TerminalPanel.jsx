        ...
        selectedNodes.map((node) => {
          return (
            <Stack key={`${node.name}_${node.providerName}`}>
              <Stack spacing={0}>
                <Box display="flex" justifyContent="center" alignItems="center" sx={{ color: blue[70] }}>
              ...
              {node.providerName && (
                <TerminalClient
                  ...
                  initialCommands={[
                    `screen -d -r $(ps -eF | awk '/._${node.name.replaceAll("_", "__")} / {print $2}') \r`,
                  ]}
                  onCtrlD={() =>
                    emitCustomEvent(EVENT_CLOSE_COMPONENT, eventCloseComponent(`${node.name}_${node.providerName}`))
                  }
                />
              )}
            </Stack>
