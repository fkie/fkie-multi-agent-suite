...
function NodeManager() {
  ...
  function onRenderTab(node /* TabNode */, renderValues /* ITabRenderValues */) {
    ...
    if (
      !['Hosts', 'Node Details', 'Packages', 'Nodes', 'Topics', 'Services', 'Parameter'].includes(
        renderValues.name
      )
    ) {
    ...
    switch (node.getId()) {
      ...
      default:
        ...
        switch (node.getConfig()?.tabType) {
          ...
          case 'node-logger':
            renderValues.leading = (
              <SettingsInputCompositeOutlinedIcon sx={{ fontSize: 'inherit', rotate: '90deg' }} />
            )
            break
          default:
            break
    ...
    children.forEach((child) => {
      if (child.getId() === LAYOUT_TABS.NODES) {
        ...
        if (window.CommandExecutor) {
          renderValues.buttons.push(<ExternalAppsModal key="external-apps-dialog" />)
        }
      }
