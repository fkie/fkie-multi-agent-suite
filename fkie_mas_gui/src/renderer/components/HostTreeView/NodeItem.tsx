...
export default function NodeItem(props: NodeItemProps): JSX.Element {
  ...
  const getLifecycleMenuItems: (node: RosNode, lifecycle?: LifecycleState) => JSX.Element[] = useCallback(
    (node, lifecycle) => {
      // add menu to change the lifecycle state
      return (
        lifecycle?.available_transitions?.map((item) => {
          return (
            <MenuItem
              ...
              >
                <Box
                  sx={{
                    ...
                    border: "3px solid",
                    borderColor: requestingLifecycle
                      ? `${colorBorder} ${colorBorder} transparent ${colorBorder}`
                      : colorBorder,
                    animation: requestingLifecycle ? "spin 0.9s linear infinite" : "none",
                    "@keyframes spin": {
            ...
            <OverflowMenu
              ...
              options={[
                ...
                }) || []),
                {
                  ...
                  key: "refresh",
                  onClick: async (): Promise<void> => {
                    if (requestingLifecycle) {
                      const err = `Lifecycle update is running for ${node.name}`;
                      logCtx.warn(err, "", err);
                      return;
                    }
                    const provider = rosCtx.getProviderById(node.providerId as string, true);
                    ...
                    if (!result?.result) {
                      ...
                      logCtx.error(err, result?.message || "", err);
                      setRequestingLifecycle(false);
                    }
                  },
                },
              ]}
  ...
  const fromTag: (tag: TTag) => JSX.Element = (tag) => {
    return (
      <Tooltip
        ...
        disableInteractive
        onClick={(event) => {
          if (tag.onClick) tag.onClick(event);
        }}
      >
  ...
  useCustomEventListener(EVENT_NODE_LIFECYCLE, (data: TEventNodeLifecycle) => {
    if (data.lifecycle.id === node.id && data.provider.id === node.providerId) {
      const info = `Lifecycle for ${node.name} updated`;
      logCtx.info(info, JSON.stringify(data.lifecycle), info);
      setRequestingLifecycle(false);
  ...
  useEffect(() => {
    const provider = rosCtx.getProviderById(node.providerId as string, true);
    setLifecycle(provider?.getLifecycleForNode(node.id));
    ...
    for (const launchInfo of node.launchInfo.values()) {
      if (launchInfo.composable_container) {
        if (launchInfo.composable_container === launchInfo.node_name) {
          setComposableTag({
            id: "Manager",
            data: "Manager",
            color: getTagColor(launchInfo.composable_container),
            tooltip: "Manager of a composable nodes",
          } as TTag);
        } else {
          setComposableTag({
            ...
            data: "Nodelet",
            color: getTagColor(launchInfo.composable_container),
            tooltip: `Composable node, container: ${launchInfo.composable_container}`,
          } as TTag);
        }
      }
    }
    ...
    if (provider) {
      ...
      if (node.guid && provider.sameIdDict[node.guid]?.length > 1) {
        setSameIdTag({
          id: node.guid,
          ...
          <Stack direction="row" display="flex" alignItems="center" justifyItems="center" paddingLeft={0.0}>
            ...
            <Stack direction="row" display="flex" alignItems="center" justifyItems="center" paddingLeft={0.0}>
              ...
              {node.status !== RosNodeStatus.RUNNING && timerPeriod.length > 0 && (
                <Tooltip
                  title={`Delayed start due to configuration ${JSON.stringify(timerPeriod)} sec`}
                  placement="left"
                >
            ...
            >
              {getLifecycleMenuItems(node, lifecycle)}
            </Menu>
          </Stack>
        }
        {...other}
