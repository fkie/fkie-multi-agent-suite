...
class MultimasterManager {
  ...
  public startMasterDiscovery: (
    ...
    let dName =
      name || versStr === '2' ? `discovery_${hostSuffix}` : 'mas_discovery';
    if (versStr === '2') {
    ...
    if (versStr === '1') {
      ...
      const dPort = `${
        Number(getArgument(ARGUMENTS.DISCOVERY_MCAST_PORT)) + (port || 0)
      }`;
      const dGroup = group || getArgument(ARGUMENTS.DISCOVERY_MCAST_GROUP);
      const dHeartbeat =
        heartbeatHz || getArgument(ARGUMENTS.DISCOVERY_HEARTBEAT_HZ);
  ...
  public startMasterSync: (
    ...
    if (versStr === '1') {
      ...
      const doNotSyncParam =
        doNotSync && doNotSync?.length > 0
          ? `_do_not_sync:=[${doNotSync?.toString()}]`
          : ' ';
      const syncTopicsParam =
        syncTopics && syncTopics?.length > 0
          ? `_sync_topics:=[${syncTopics?.toString()}]`
          : ' ';
      let cmdMasterSync = '';
      if (versStr === '1') {
   ...
   */
  public startBackgroundProcesses = async () => {
    try {
