...
from fkie_mas_pylib.defines import LOG_PATH, SCREEN, SCREEN_NAME_MAX_CHARS, SCREEN_SLASH_SEP, SETTINGS_PATH
...
def create_session_name(node: str = "", namespace: str = "/") -> str:
    ...
    result = ns_join(namespace, node).replace(SCREEN_SLASH_SEP, "%s%s" % (SCREEN_SLASH_SEP, SCREEN_SLASH_SEP))
...
def session_name2node_name(session: str) -> str:
    ...
    :rtype: str
    """
    node_name = session.replace("%s%s" % (SCREEN_SLASH_SEP, SCREEN_SLASH_SEP), "//")
...
def get_active_screens(nodename: str = "") -> dict[str, str]:
    ...
    ps = SupervisedPopen([SCREEN, "-ls"], stdout=subprocess.PIPE, object_id=f"get_active_screens for {nodename}")
    output = ps.stdout.read() if sys.version_info[0] <= 2 else str(ps.stdout.read(), "utf-8")
    if output:
        if time.time() - start_time > 1.0:
            Log.warn(
                "'%s -ls' took too long (%.3f sec)! Fix your network configuration!"
...
def get_ros_logfile(node: str, namespace: str = "/", for_new_screen: bool = False) -> str:
    ...
    if node is not None:
        ...
        logfile = "%s.log" % os.path.join(LOG_PATH, nodename.strip("/").replace("/", "_"))
        if os.path.exists(logfile) or for_new_screen:
            ...
            if not os.path.exists(log_path):
                return ""
            p = re.compile(r"%s-\d*.log" % (nodename.strip("/").replace("/", "-")))
            files = os.listdir(log_path)
            for fn in files:
                if p.match(fn):
                    return os.path.join(log_path, fn)
            p = re.compile(r"%s-\d*-stdout.log" % (nodename.strip("/").replace("/", "-")))
            for fn in files:
                if p.match(fn):
                    return os.path.join(log_path, fn)
    return logfile
