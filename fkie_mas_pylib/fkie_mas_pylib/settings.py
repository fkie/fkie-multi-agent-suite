            ...
            "sysmon": {
                "CPU": {
                    "load_warn_level": {
                        ...
                        ":min": 0,
                        ":max": 1.0,
                        ":hint": "warn if more than one CPU core exceeds this percentage",
                    },
                },
                "Disk": {
                    "usage_warn_level": {
                        ...
                        ":min": 0,
                        ":max": 1.0,
                        ":hint": "warn if used space exceeds this percentage",
                    },
                ...
                "Memory": {
                    "usage_warn_level": {
                        ...
                        ":min": 0,
                        ":max": 1.0,
                        ":hint": "warn if used memory exceeds this percentage",
                    },
                },
                "Network": {
                    "load_warn_level": {
                        ...
                        ":max": 1.0,
                        ":default": 0.9,
                        ":hint": "warn if load exceeds the percentage of the maximum speed",
                    },
                    "speed": {
                        ":value": 6,
                        ":default": 6,
            ...
            if base_name in cfg_item:
                if isinstance(cfg_item[base_name], dict):
                    if self._is_writable(cfg_item[base_name]):
                        ...
                        raise Exception(f"{param_name} is a read only parameter!")
                else:
        ...
        with self._mutex:
            try:
                ...
                with open(self.filename) as stream:
                    ...
                    if result is None:
                        Log.info("reset configuration file %s" % self.filename)
                        self.save()
                    else:
                        Log.info("loaded configuration from %s" % self.filename)
                        self._cfg = self._apply_recursive(result, self._cfg)
            except (OSError, ruamel.yaml.YAMLError) as exc:
    ...
    def save(self):
        ...
        with open(self.filename, "w") as stream:
            try:
                ...
                Log.warn(f"Cant't save configuration to '{self.filename}': {exc}")
