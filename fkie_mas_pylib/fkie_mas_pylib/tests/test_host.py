...
class TestHost(unittest.TestCase):
    ...
    def test_get_hostname(self):
        ...
        self.assertEqual(hostname, "", "Hostname from `` should be ``, got: %s, expected: ''" % (hostname))
        name = "myhost"
        ...
        self.assertEqual(
            hostname, name, "Wrong hostname from `%s:11111`, got: %s, expected: %s" % (name, hostname, name)
        )
        wrong = "%s:11:2" % name
    ...
    def test_get_ros_hostname(self):
        ...
        self.assertEqual(roshn, "", "ros hostname from `None` should be ``, got: %s, expected: ''" % (roshn))
        roshn = host.get_ros_hostname("http://myhost:11311")
        self.assertEqual(
            roshn,
            "myhost",
            "wrong ros hostname from `'http://myhost:11311'`, got: %s, expected: %s" % (roshn, "myhost"),
        )
