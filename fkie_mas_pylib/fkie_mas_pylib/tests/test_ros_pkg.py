...
class TestRosPkgLib(unittest.TestCase):
    ...
    def test_get_packages(self):
        ...
        self.assertEqual(
            count_exp,
            len(pkg_res),
            "wrong count of get_packages(%s), expected: %d, got: %d -> packages: %s"
            % (path, count_exp, len(pkg_res), pkg_res),
        )
    ...
    def test_get_cwd(self):
        ...
        self.assertEqual(
            test_path, result_path, "wrong get_cwd from 'node' type, expected: %s, got: %s" % (test_path, result_path)
        )
        test_path = os.getcwd()
        result_path = ros_pkg.get_cwd("cwd", "")
        self.assertEqual(
            test_path, result_path, "wrong get_cwd from 'cwd' type, expected: %s, got: %s" % (test_path, result_path)
        )
        ...
        self.assertEqual(
            test_path, result_path, "wrong get_cwd from empty type, expected: %s, got: %s" % (test_path, result_path)
        )

    def test_package_name(self):
        pkg = ros_pkg.get_name(self.pkg_path)
        self.assertEqual(PKG, pkg[0], "wrong package name, expected: %s, got: %s" % (PKG, pkg[0]))
        pkg_dir = self.pkg_path
        self.assertEqual(pkg_dir, pkg[1], "wrong package path, expected: %s, got: %s" % (pkg_dir, pkg[1]))
        # test cache
        pkg = ros_pkg.get_name(self.pkg_path)
        self.assertEqual(PKG, pkg[0], "wrong package name from cache, expected: %s, got: %s" % (PKG, pkg[0]))
        self.assertEqual(pkg_dir, pkg[1], "wrong package path from cache, expected: %s, got: %s" % (pkg_dir, pkg[1]))
        # test invalid path
        pkg = ros_pkg.get_name("INVALID")
        self.assertEqual("", pkg[0], "wrong package name, expected: %s, got: %s" % (None, pkg[0]))
        self.assertEqual("", pkg[1], "wrong package name, expected: %s, got: %s" % (None, pkg[1]))
