...
class TestScreen(unittest.TestCase):
    ...
    def test_create_session_name(self):
        ...
        self.assertEqual(name, "", f"wrong screen session name from `None`, got: {name}, expected: ''")
        name = screen.create_session_name("/test/node")
        self.assertEqual(
            name,
            TEST_NODE_NAME,
            f"wrong screen session name from `/test/node`, got: {name}, expected: {TEST_NODE_NAME}",
        )
    ...
    def test_split_session_name(self):
        ...
        self.assertEqual(name, "", f"wrong screen session name after split from `None`, got: {name}, expected: ''")
        _pid, name = screen.split_session_name(f"123.{TEST_NODE_NAME}")
        self.assertEqual(
            name,
            TEST_NODE_NAME,
            f"wrong screen session name after split from `123.{TEST_NODE_NAME}`, got: {name}, expected: {TEST_NODE_NAME}",
        )
        ...
        self.assertEqual(name, "", f"wrong name after screen split session `666.`, got: {name}, expected: ''")
