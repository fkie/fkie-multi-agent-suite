...
class TestLaunchXmlLib(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_include_file = "%s/include_dummy.launch" % os.path.join(self.pkg_path, self.res_dir)
        self.test_wrong_include_file = "%s/test_wrong_include.launch" % os.path.join(self.pkg_path, self.res_dir)

    def tearDown(self):
        pass

    def test_includefile_repr(self):
        inc_file = xml.IncludedFile("path_or_str", 1, "inc_path", False, "raw_inc_path", 0, {"one": "empty"}, -1)
        ...
        self.assertEqual(
            inc_file_str,
            "%s" % inc_file,
            "wrong _repr__ from IncludedFile, expected: %s, got: %s" % (inc_file_str, "%s" % inc_file),
        )
    ...
    def test_interpret_path(self):
        ...
        self.assertEqual(
            ...
            path,
            "wrong interpreted path, expected: %s, got: %s" % (self.test_include_file, path),
        )
        ...
        self.assertEqual(
            self.test_include_file,
            path,
            "wrong interpreted path with replace the subdirectory by `///`, expected: %s, got: %s"
    ...
    def test_get_arg_names(self):
        ...
        self.assertEqual(
            "test_name_arg", args[0], "wrong get_arg_names, expected: %s, got: %s" % ("test_name_arg", args[0])
        )
        ...
        self.assertEqual(
            "test_name_second", args[1], "wrong get_arg_names, expected: %s, got: %s" % ("test_name_second", args[1])
        )

    def test_include_files(self):
        file_list = [file_tuple for file_tuple in xml.find_included_files(self.test_include_file, unique=True)]
        self.assertEqual(
            5, len(file_list), "Count of unique included files is wrong, expected: %d, got: %d" % (5, len(file_list))
        )
        file_list = [
            file_tuple for file_tuple in xml.find_included_files(self.test_include_file, recursive=False, unique=True)
        ]
        self.assertEqual(
            3,
            len(file_list),
            "Count of unique included files while not recursive search is wrong, expected: %d, got: %d"
            % (3, len(file_list)),
        )
        file_list = [file_tuple for file_tuple in xml.find_included_files(self.test_include_file, unique=False)]
        self.assertEqual(
            10, len(file_list), "Count of included files is wrong, expected: %d, got: %d" % (10, len(file_list))
        )
        self.assertEqual(
            6,
            file_list[0].line_number,
            "Wrong line number of first included file, expected: %d, got: %d" % (6, file_list[0].line_number),
        )
        self.assertEqual(
            10,
            file_list[2].line_number,
            "Wrong line number of second included file, expected: %d, got: %d" % (10, file_list[2].line_number),
        )
        no_file_list = [file_tuple for file_tuple in xml.find_included_files(self.test_wrong_include_file, unique=True)]
        self.assertEqual(
            0,
            len(no_file_list),
            "Count of 'wrong' included files is wrong, expected: %d, got: %d" % (0, len(no_file_list)),
        )
