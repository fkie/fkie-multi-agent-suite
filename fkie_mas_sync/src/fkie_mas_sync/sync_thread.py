...
import rospy
from fkie_mas_discovery.common import get_hostname
from fkie_mas_discovery.filter_interface import FilterInterface
from fkie_mas_pylib.logging.logging import Log
from fkie_mas_pylib.system import ros1_masteruri

from fkie_mas_msgs.msg import SyncMasterInfo, SyncServiceInfo, SyncTopicInfo
...
class SyncThread:
    ...
    def __init__(self, name, uri, discoverer_name, monitoruri, timestamp, sync_on_demand=False, callback_resync=None):
        ...
        self.timestamp_local = 0.0
        self.timestamp_remote = 0.0
        self._online = True
        self._offline_ts = 0

        self.masteruri_local = ros1_masteruri.from_ros()
        self.hostname_local = get_hostname(self.masteruri_local)
        Log.debug("SyncThread[%s]: create this sync thread, discoverer_name: %s", self.name, self.discoverer_name)
        ...
        self._filter.load(
            ...
            [
                "bond/Status",
                "fkie_mas_msgs/SyncTopicInfo",
                "fkie_mas_msgs/SyncServiceInfo",
                "fkie_mas_msgs/SyncMasterInfo",
        ...
        with self.__lock_info:
            if self.__sync_info is None:
                ...
                for s_n, s_uri, n_n, n_uri in self.__services:
                    ...
                    result_set.add(n_n)
                self.__sync_info = SyncMasterInfo(
                    self.uri, list(result_set), result_publisher, result_subscriber, result_services
                )
            return self.__sync_info

    def set_online(self, value, resync_on_reconnect_timeout=0.0):
        if value:
            if not self._online:
                with self.__lock_intern:
                    ...
                    if offline_duration >= resync_on_reconnect_timeout:
                        Log.info(
                            "SyncThread[%s]: perform resync after the host was offline (unregister and register again to avoid connection losses to python topic. These does not suppot reconnection!)",
                            self.name,
                        )
                        ...
                        Log.info(
                            "SyncThread[%s]: skip resync after the host was offline because of resync_on_reconnect_timeout=%.2f and the host was only %.2f sec offline",
                            self.name,
                            resync_on_reconnect_timeout,
                            offline_duration,
                        )
        else:
            self._online = False
    ...
    def update(self, name, uri, discoverer_name, monitoruri, timestamp):
        ...
        with self.__lock_intern:
            ...
            if self.timestamp_local != timestamp:
                Log.debug(
                    "SyncThread[%s]: update notify new timestamp(%.9f), old(%.9f)",
                    self.name,
                    timestamp,
    ...
    def set_own_masterstate(self, own_state, sync_on_demand=False):
        ...
        @type own_state:  C{fkie_mas_discovery/MasterInfo}
        ...
        with self.__lock_intern:
            ...
            if self.__own_state is None or (self.__own_state.timestamp_local != timestamp_local):
                ...
                Log.debug(
                    "SyncThread[%s]: local state update notify new timestamp(%.9f), old(%.9f)",
                    self.name,
                    timestamp_local,
                    ownstate_ts,
                )
    ...
    def _request_remote_state(self, handler):
        ...
        try:
            ...
            Log.info("SyncThread[%s] Requesting remote state from '%s'", self.name, self.monitoruri)
            ...
            Log.error("SyncThread[%s] ERROR: %s", self.name, traceback.format_exc())
        finally:
            self.__on_update = False
    ...
    def _apply_remote_state(self, remote_state):
        ...
        try:
            Log.debug("SyncThread[%s]: remote state: %s" % (self.name, remote_state))
            ...
            for topic, topictype, node, nodeuri in set(self.__publisher) - set(publisher):
                ...
                Log.debug("SyncThread[%s]: prepare UNPUB %s[%s] %s", self.name, node, nodeuri, topic)
            ...
            for topic, topictype, node, nodeuri in publisher_to_register:
                ...
                Log.debug("SyncThread[%s]: prepare PUB %s[%s] %s[%s]", self.name, node, nodeuri, topic, topictype)
            ...
            for topic, nodes in subscribers:
                for node in nodes:
                    ...
                    if topictype and nodeuri and not self._do_ignore_nts(node, topic, topictype):
                        ...
                        if (topic, node, nodeuri) not in self.__subscriber:
            ...
            for topic, topictype, node, nodeuri in set(self.__subscriber) - set(subscriber):
                ...
                Log.debug("SyncThread[%s]: prepare UNSUB %s[%s] %s", self.name, node, nodeuri, topic)
            ...
            for topic, topictype, node, nodeuri in subscriber_to_register:
                ...
                Log.debug("SyncThread[%s]: prepare SUB %s[%s] %s[%s]", self.name, node, nodeuri, topic, topictype)
            ...
            for service, serviceuri, node, nodeuri in set(self.__services) - set(services):
                ...
                Log.debug("SyncThread[%s]: prepare UNSRV %s[%s] %s[%s]", self.name, node, nodeuri, service, serviceuri)
            ...
            for service, serviceuri, node, nodeuri in services_to_register:
                ...
                Log.debug("SyncThread[%s]: prepare SRV %s[%s] %s[%s]", self.name, node, nodeuri, service, serviceuri)
            ...
            if not self.__unregistered:
                ...
                Log.debug("SyncThread[%s]: current timestamp %.9f, local %.9f", self.name, stamp, stamp_local)
                if self.timestamp_remote > stamp_local:
                    Log.debug("SyncThread[%s]: invoke next update, remote ts: %.9f", self.name, self.timestamp_remote)
                    self._update_timer = threading.Timer(
                        random.random() * 2.0, self._request_remote_state, args=(self._apply_remote_state,)
                    )
                    self._update_timer.start()
            # check md5sum for topics
            with self.__lock_info:
                self._check_md5sums(publisher_to_register + subscriber_to_register)
            # check if remote mas_sync was stopped
            ...
            for h, (code, statusMessage, r) in zip(handler, mresult):
                try:
                    if h[0] == "sub":
                        if code == -1:
                            Log.warn(
                                "SyncThread[%s]: topic subscription error: %s (%s), %s %s, node: %s",
                                self.name,
                                h[1],
                                h[2],
                                str(code),
                            ...
                            Log.debug(
                                "SyncThread[%s]: topic subscribed: %s, %s %s, node: %s",
                                self.name,
                                h[1],
                                str(code),
                                str(statusMessage),
                    ...
                    if h[0] == "sub" and code == 1 and len(r) > 0:
                        ...
                        if code == -1:
                            Log.warn(
                                "SyncThread[%s]: topic advertise error: %s (%s), %s %s",
                                self.name,
                                h[1],
                                h[2],
                                str(code),
                            ...
                            Log.debug(
                                "SyncThread[%s]: topic advertised: %s, %s %s",
                                self.name,
                                h[1],
                                str(code),
                                str(statusMessage),
                        ...
                        Log.debug(
                            "SyncThread[%s]: topic unsubscribed: %s, %s %s",
                            self.name,
                            h[1],
                            str(code),
                            str(statusMessage),
                        ...
                        Log.debug(
                            "SyncThread[%s]: topic unadvertised: %s, %s %s",
                            self.name,
                            h[1],
                            str(code),
                            str(statusMessage),
                        ...
                        if code == -1:
                            Log.warn(
                                "SyncThread[%s]: service registration error: %s, %s %s",
                                self.name,
                                h[1],
                                str(code),
                                str(statusMessage),
                            ...
                            Log.debug(
                                "SyncThread[%s]: service registered: %s, %s %s",
                                self.name,
                                h[1],
                                str(code),
                                str(statusMessage),
                        ...
                        Log.debug(
                            "SyncThread[%s]: service unregistered: %s, %s %s",
                            self.name,
                            h[1],
                            str(code),
                            str(statusMessage),
                    ...
                    Log.error(
                        "SyncThread[%s] ERROR while analyzing the results of the registration call [%s]: %s",
                        self.name,
                        h[1],
    ...
    def perform_resync(self):
        ...
        with self.__lock_info:
            ...
            for topic, topictype, node, nodeuri in self.__subscriber:
                own_master_multi.registerSubscriber(node, topic, topictype, nodeuri)
                Log.debug("SyncThread[%s]: prepare RESUB %s[%s] %s[%s]", self.name, node, nodeuri, topic, topictype)
                handler.append(("sub", topic, topictype, node, nodeuri))
            # reregister publishers
            for topic, topictype, node, nodeuri in self.__publisher:
                own_master_multi.registerPublisher(node, topic, topictype, nodeuri)
                Log.debug("SyncThread[%s]: prepare REPUB %s[%s] %s[%s]", self.name, node, nodeuri, topic, topictype)
                handler.append(("pub", topic, topictype, node, nodeuri))
        result = own_master_multi()
        self._check_multical_result(result, handler)
    ...
    def _check_md5sums(self, topics_to_register):
        try:
            ...
            if self._use_md5check_topics:
                Log.info("SyncThread[%s] Requesting remote md5sums '%s'", self.name, self.monitoruri)
                            ...
                            for topicname, topictype, node, nodeuri in topics_to_register:
                                if topictype == rttype:
                                    if (topicname, node, nodeuri) not in self._md5warnings:
                                        if lmd5sum is None:
                                            Log.warn(
                                                "Unknown message type %s for topic: %s, local host: %s, remote host: %s"
                                                % (rttype, topicname, self.hostname_local, self.name)
                                            )
                                        else:
                                            Log.warn(
                                                "Different checksum detected for topic: %s, type: %s, local host: %s, remote host: %s"
                            ...
                            if own_topictype not in ["*", None] and topictype not in ["*", None]:
                                if topictype != own_topictype:
                                    if (topicname, node, nodeuri) not in self._topic_type_warnings:
                                        Log.warn(
                                            "Different topic types detected for topic: %s, own type: %s remote type: %s, local host: %s, remote host: %s"
                                            % (topicname, own_topictype, topictype, self.hostname_local, self.name)
                                        )
                                        self._topic_type_warnings[(topicname, node, nodeuri)] = (
                                            "local: %s, remote: %s" % (own_topictype, topictype)
                                        )
                    except Exception as err:
                        import traceback
    ...
    def _unreg_on_finish(self):
        with self.__lock_info:
            ...
            try:
                Log.debug("    SyncThread[%s] clear all registrations", self.name)
                socket.setdefaulttimeout(5)
                own_master = xmlrpcclient.ServerProxy(self.masteruri_local)
                own_master_multi = xmlrpcclient.MultiCall(own_master)
                # end routine if the master was removed
                for topic, _topictype, node, uri in self.__subscriber:
                    Log.debug("    SyncThread[%s]   unsibscribe %s [%s]" % (self.name, topic, node))
                ...
                for topic, _topictype, node, uri in self.__publisher:
                    Log.debug("    SyncThread[%s]   unadvertise %s [%s]" % (self.name, topic, node))
                    own_master_multi.unregisterPublisher(node, topic, uri)
                for service, serviceuri, node, uri in self.__services:
                    Log.debug("    SyncThread[%s]   unregister service %s [%s]" % (self.name, service, node))
                    own_master_multi.unregisterService(node, service, serviceuri)
                Log.debug("    SyncThread[%s] execute a MultiCall", self.name)
                _ = own_master_multi()
                Log.debug("    SyncThread[%s] finished", self.name)
            except:
                Log.error("SyncThread[%s] ERROR while ending: %s", self.name, traceback.format_exc())
            socket.setdefaulttimeout(None)
    ...
    def _get_nodeuri(self, node, nodes, remote_masteruri):
        for nodename, uri, masteruri, pid, local in nodes:
            if (nodename == node) and (
                (self._filter.sync_remote_nodes() and masteruri == remote_masteruri) or local == "local"
            ):
