...
import fkie_mas_discovery.interface_finder as interface_finder
import rospy
from diagnostic_msgs.msg import DiagnosticArray, DiagnosticStatus, KeyValue
from fkie_mas_discovery.common import create_pattern, get_hostname, is_empty_pattern, read_interface, resolve_url
from fkie_mas_discovery.master_info import MasterInfo
...
from fkie_mas_msgs.msg import MasterState
from fkie_mas_msgs.srv import DiscoverMasters, GetSyncInfo, GetSyncInfoResponse
...
class Main:
    ...
    def __init__(self):
        """
        Creates a new instance. Find the topic of the mas_discovery node using
        U{fkie_mas_discovery.interface_finder.get_changes_topic()
        <http://docs.ros.org/api/fkie_mas_discovery/html/modules.html#interface-finder-module>}.
        ...
        topic_names = interface_finder.get_changes_topic(ros1_masteruri.from_master(), check_host=self._check_host)
    ...
    def _rosmsg_callback_master_state(self, data):
        ...
        @type data: U{fkie_mas_discovery.MasterState
        <http://docs.ros.org/api/fkie_mas_msgs/html/msg/MasterState.html>}
        """
        with self.__lock:
            if not rospy.is_shutdown():
                if data.state in [MasterState.STATE_REMOVED]:
                    ...
                    self.update_master(
                        m.name,
                        m.uri,
                        m.last_change.to_sec(),
                        m.last_change_local.to_sec(),
                        m.discoverer_name,
                        m.monitoruri,
                        m.online,
                    )
    ...
    def obtain_masters(self):
        ...
        @see: U{fkie_mas_discovery.interface_finder.get_listmaster_service()
            <http://docs.ros.org/api/fkie_mas_discovery/html/modules.html#interface-finder-module>}
        """
        if not rospy.is_shutdown():
                ...
                try:
                    with self.__lock:
                        try:
                            ...
                            Log.info("ROS masters obtained from '%s': %s", service_name, master_names)
                            ...
                            Log.warn("ERROR Service call 'list_masters' failed: %s", str(e))
                except:
                    import traceback

                    Log.warn("ERROR while initial list masters: %s", traceback.format_exc())
                finally:
    ...
    def update_master(self, mastername, masteruri, timestamp, timestamp_local, discoverer_name, monitoruri, online):
        ...
        try:
                        ...
                        if online:
                            if mastername in self.masters:
                                ...
                                self.masters[mastername] = SyncThread(
                                    ...
                                    callback_resync=self._callback_perform_resync,
                                )
            ...
            Log.warn("ERROR while update master[%s]: %s", str(mastername), traceback.format_exc())
    ...
    def get_own_state(self, monitoruri):
        ...
        try:
            ...
            Log.warn("ERROR while getting own state from '%s': %s", monitoruri, traceback.format_exc())
            socket.setdefaulttimeout(None)
            time.sleep(3)
    ...
    def remove_master(self, ros_master_name):
        ...
        try:
            ...
            Log.warn("ERROR while removing master[%s]: %s", ros_master_name, traceback.format_exc())
    ...
    def _threading_stop_sync(self, sync_thread, ident):
        if isinstance(sync_thread, SyncThread):
            ...
            Log.info("  Finished synchronization to `%s`" % sync_thread.name)
            del sync_thread
    ...
    def finish(self, msg=""):
        ...
        while len(self._join_threads) > 0:
            Log.info("  Wait for ending of %s threads ...", str(len(self._join_threads)))
            time.sleep(1)
    ...
    def _load_interface(self):
        ...
        try:
            ...
            Log.info("sync_topics_on_demand: %s", self.__sync_topics_on_demand)
            self.__resync_on_reconnect = rospy.get_param("~resync_on_reconnect", True)
            Log.info("resync_on_reconnect: %s", self.__resync_on_reconnect)
            self.__resync_on_reconnect_timeout = rospy.get_param("~resync_on_reconnect_timeout", 0)
            Log.info("resync_on_reconnect_timeout: %s", self.__resync_on_reconnect_timeout)
        except:
            import traceback
