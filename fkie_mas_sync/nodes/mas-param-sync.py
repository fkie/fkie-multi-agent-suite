...
def master_changed(msg, cb_args):
    ...
    if msg.master.uri != ros1_masteruri.from_master() and local_name in param_cache:
        ...
        if not __add_ns:
            for key in ['run_id', 'rosversion', 'roslaunch', 'rosdistro', 'mas_sync', 'mas_discovery', 'master_sync', 'master_discovery', 'capabilities', 'mastername', 'robots']:
                try:
                    del params_from[key]
                except Exception:
                    pass
...
def main():
    ...
    sub = rospy.Subscriber('mas_discovery/changes', MasterState, master_changed,
