...
def master_changed(msg, cb_args):
    ...
    if msg.master.uri != ros1_masteruri.from_master() and local_name in param_cache:
        ...
        Log.debug(f"Getting params from {msg.master.uri}...")
        ...
        if __only:
            for key in params_from.keys():
                if key not in __only:
                    del params_from[key]
        Log.debug(f"Syncing params from {msg.master.name} to {local_name}...")
        if __add_ns:
            _ns = msg.master.name
        else:
            _ns = ""
        Log.debug(f"Got {len(params_from)} params.")
        if param_cache.get(_ns, None) != params_from:
            ...
            Log.debug(f"Done syncing params from {msg.master.name} to {local_name}.")
        else:
            Log.debug(f"Params have not changed from {msg.master.name} to {local_name}.")
    else:
        local_name = msg.master.name
        local_master.append(local_name)
        master_from = rospy.MasterProxy(msg.master.uri)
        Log.debug(f"Getting params from local {msg.master.uri}...")
        param_cache[local_name] = master_from.getParam("/")[2]
        Log.debug(f"Got {len(param_cache[local_name])} local params.")
